                            ...
                            if object_name[-4:] == '.txt':
                                ...
                                try:
                                    with open(object_file, 'r') as f:
                                        # TODO: find the file which has ilegal input
                                        print(object_file)
                                        # add strip() to avoid Area_5/hallway_6/Annotations/ceiling_1.txt's line 180389 has a "^@" string, which result in conversion error(i.e.,ValueError: could not convert string to float: '185\x00187')
                                        object_data = np.array([[float(x) for x in line.strip().split()] for line in f], dtype="object") # Stack all data
