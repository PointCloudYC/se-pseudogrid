...
from utils.util import AverageMeter, accuracy, reduce_tensor, str2bool
...
def parse_option():
    parser = argparse.ArgumentParser('PipeWork classification training')
    ...
    config.log_dir = os.path.join(args.log_dir, 'pipework', f'{ddir_name}_{int(time.time())}')
...
def get_loader(args):
    ...
    train_dataset = PipeWorkCls(input_features_dim=config.input_features_dim, num_points=args.num_points,
...
if __name__ == "__main__":
    ...
    logger = setup_logger(output=config.log_dir, distributed_rank=dist.get_rank(), name="pipework")
    if dist.get_rank() == 0:
