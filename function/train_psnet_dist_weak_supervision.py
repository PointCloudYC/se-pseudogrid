...
def parse_option():
    ...
    parser.add_argument("--weak_ratio", type=float, help='weak ratio for weakly supervised learning')
    ...
    if args.weak_ratio:
        config.weak_ratio = args.weak_ratio
...
def get_loader(config, weak_ratio_val = 1.0):
    # set the data loader
    ...
    train_dataset = PSNetSeg(input_features_dim=config.input_features_dim,
                        ...
                        split='train', weak_ratio=config.weak_ratio)
    #  during testing all points are tested so weak ratio is 1 rather than weak_ratio(e.g. 0.001)
    val_dataset = PSNetSeg(input_features_dim=config.input_features_dim,
                        ...
                        split='val', weak_ratio=weak_ratio_val)
...
def train(epoch, train_loader, model, criterion, optimizer, scheduler, config):
    ...
    for idx, (points, mask, features, points_labels, cloud_label, input_inds, weak_mask) in enumerate(train_loader):
        data_time.update(time.time() - end)
        bsz = points.size(0)
        # forward
        points = points.cuda(non_blocking=True) # B,N,3, B*N points
        features = features.cuda(non_blocking=True) # B,N,3
        points_labels = points_labels.cuda(non_blocking=True) # B,N

        # handle masking mechinism, where mask denote whether is padded point and weak_mask denote whether has labels.
        mask_final = mask * weak_mask
        # mask = mask.cuda(non_blocking=True) # B,N, about 0.5(B*N) normal pts
        # weak_mask is int64 type, so change to int32(i.e. int)
        mask_final = mask_final.int().cuda(non_blocking=True) # B,N, about 0.02(B*N) weak pts

        pred = model(points, mask_final, features) # B, N, num_classes
        loss = criterion(pred, points_labels, mask_final)
...
def validate(epoch, test_loader, model, criterion, runing_vote_logits, config, num_votes=10):
    ...
    with torch.no_grad():
        ...
        for v in range(num_votes):
            ...
            predictions = []
            targets = []
            for idx, (points, mask, features, points_labels, cloud_label, input_inds, weak_mask) in enumerate(test_loader):
                # augment for voting
                if v > 0:
                    points = RT(points)
                    points = TS(points)
                ...
                # mask = mask.cuda(non_blocking=True) # B,N, about 0.5(B*N) normal pts
                # weak_mask is int64 type, so change to int32(i.e. int)
                weak_mask = weak_mask.int().cuda(non_blocking=True)
                mask_final = mask_final.int().cuda(non_blocking=True) # B,N, about 0.02(B*N) weak pts
...
if __name__ == "__main__":
    ...
    torch.cuda.set_device(config.local_rank) # 0 by default
    ...
    logger = setup_logger(output=config.log_dir, distributed_rank=dist.get_rank(), name="psnet5")
    if dist.get_rank() == 0:
